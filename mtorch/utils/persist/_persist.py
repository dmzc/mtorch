...
def __dump_json_item(obj: Any, orjson_enabled: bool = True) -> bytes:
...
class PersistService:
    ...
    def save_json_stream(
        ...
        # 3. 确保目录存在
        PersistService.ensure_dir_exist(parent_foleder)
        ...
        def _do_write(io: BinaryIO):
            ...
            for items in iteratable_obj:
                ...
                if is_single:
                    ...
                    io.write(__dump_json_item(items, orjson_enabled=ORJSON_ENABLED))
                else:
                    is_item_first = True
                    for item in items:
                        if not is_item_first:
                            io.write(b",")
                        is_item_first = False
                        io.write(__dump_json_item(item, orjson_enabled=ORJSON_ENABLED))
            io.write(b"]")
    ...
    def save_json(
        ...
        # 1. 序列化
        json_bytes = __dump_json_item(obj=obj, orjson_enabled=pkg_enabled("orjson"))
    ...
    @staticmethod
    def save_pkl(obj: Any, file: str | Path) -> Path:
        r"""
        将可序列化Python对象保存为pickle二进制文件。
        支持嵌套numpy ndarray；会完整保留对象内部结构、dtype与形状，无数值精度损失。
        使用pickle最高协议序列化；输出模式为二进制写入。

        Warning:
            禁止加载来源不可信的pickle文件，反序列化可执行任意恶意代码。

        Args:
            obj: 需要持久化的Python对象，支持字典、列表、类实例、numpy数组等
            out_path: 输出pickle文件路径

        Returns:
            None
        """
        target = Path(file)
        PersistService.ensure_dir_exist(target.parent)
        import pickle
    ...
    @staticmethod
    def load_pkl(file: str | Path) -> Any:
        r"""
        从pickle二进制文件反序列化，还原原始Python对象。
        可读取包含numpy ndarray的pickle；类实例加载时要求当前作用域存在对应类定义。
    ...
    @staticmethod
    def tar(src_files: list[Path], target_file: Path) -> None:
        r"""
        将一组文件/目录归档打包，**默认启用gzip压缩，输出tar.gz**。
        列表内可混合文件与目录，目录会递归打包全部子内容；归档内会携带原始父目录路径。

        Notes:
            1. src_file传递的文件，会把``Path.name``当作arcname，此时写入到的
            是顶级目录，src_file传递的目录，此时把目录写到顶级目录。

        Args:
            src_files: 待归档的源文件/目录 Path 对象列表
            target_file: 输出归档文件路径，通常后缀为 .tar.gz

        Returns:
            None
        """
        PersistService.ensure_dir_exist(target_file.parent)
        import tarfile

        with tarfile.open(name=target_file, mode="w:gz") as tar:
            for src_file in src_files:
                ...
                if not src_file.exists():
                    ...
                    warnings.warn(
                        f"{str(src_file)} not exists!", category=RuntimeWarning
                    )
                    continue
                tar.add(src_file, arcname=src_file.name)

    @staticmethod
    def utar(tar_file: Path, target_dir: Path) -> None:
        r"""
        解包 tar 归档文件，自动识别裸tar / tar.gz / tar.bz2。
        将归档内部全部内容解压输出到目标目录；归档保存的原始目录结构会完整还原；目标目录不存在时自动创建。
        如果输入不是合法tar归档，会抛出tarfile.ReadError。

        Args:
            src_file: tar / tar.gz / tar.bz2 归档文件路径
            target_dir: 解压输出的目标目录

        Returns:
            None
        """
        PersistService.ensure_dir_exist(target_dir)

        import tarfile
        from tarfile import TarFile

        with tarfile.open(name=tar_file, mode="r") as ftar:
            ftar: TarFile
            ftar.extractall(path=target_dir)

    @staticmethod
    def get_tar_files(tar_file: Path) -> list[str]:
        import tarfile
        from tarfile import TarFile

        items: list[str]
        with tarfile.open(name=tar_file.resolve(), mode="r:*") as ftar:
            ftar: TarFile
            items = [item for item in ftar.getnames()]
        return items

    @staticmethod
    def is_file_in_tar(file: str, tar_file: Path) -> bool:
        if not tar_file.exists():
            raise RuntimeError(f"{str(tar_file)} not exists!")
        if not tar_file.is_file():
            raise RuntimeError(f"{str(tar_file)} not a tar file!")
        return file in PersistService.get_tar_files(tar_file=tar_file)
