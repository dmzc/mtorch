...
from mtorch.tests.utils import TEST_DIRS, mock_missing_package
import numpy as np
...
def test_tar():
    ...
    for item in output_dir.rglob("*"):
        items.append(str(item.relative_to(output_dir)))
    assert items == ["a1.txt", "b", "b\\b1.txt"], "目录和文件能一起正常归档"
    
    
    with open(output_dir/"b/b1.txt","r") as reader:
        assert reader.read()=="这是a/b目录下的b1.txt文件","读出来的文件是正常的"
